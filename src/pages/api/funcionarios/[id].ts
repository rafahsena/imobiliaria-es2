...
import { getFuncionario, alterarFuncionario, deletarFuncionario } from "src/backend/funcionario/ctrFuncionario";
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (req.method === "DELETE") {
        try {
            const funcionario = await deletarFuncionario(Number(id));
            res.status(200).json({});
        } catch (e) {
